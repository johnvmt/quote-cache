...
import PouchDBCollection from "../src/collections/PouchDBCollection";
import DBCollection from "../src/collections/DBCollection";

const confFile = process.env.POUCHDB_TEST_CONFIG;
if (!confFile)
	throw new Error('"POUCHDB_TEST_CONFIG" not found in environment');
...
describe(`Processing "${__filename}"`, () => {
	...
	describe('Primitive-type item functions', () => {
		it('Set new itemID, no TTL', async () => {
			...
			const itemID = 'ID';
			const insertedItemValue = {key: 'VALUE'};

			await collection.upsertItem(itemID, insertedItemValue);

			let savedItemValue = await collection.getItem(itemID);

			if(savedItemValue.key !== insertedItemValue.key)
		...
		it('Set new itemID, increment value (using numbers)', async () => {
			...
			if(savedItemValue !== insertedItemValue + increment)
...
function createCollection() {
	return new PouchDBCollection(config);
}
